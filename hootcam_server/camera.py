...
class DualCameraService:
    ...
    Main stream → H.264 encoder → CircularOutput2 (pre-capture buffer).
    ...
    def __init__(
        ...
        self._started = False
        self._config0: Optional[Tuple[int, int, int, float]] = None  # w, h, fps, pre_capture_sec
        self._config1: Optional[Tuple[int, int, int, float]] = None
    ...
    def start(
        ...
        try:
            ...
            self._config0 = (width0, height0, framerate0, pre_capture_sec0)
            cam0_ok = True
        ...
        try:
            ...
            self._config1 = (width1, height1, framerate1, pre_capture_sec1)
            cam1_ok = True
    ...
    def stop(self) -> None:
        """Stop recording and stop both cameras."""
    ...
    def restart_camera(self, camera_index: int) -> bool:
        """Stop and start one camera. Returns True if restart succeeded."""
        if Picamera2 is None:
            return False
        cfg0 = self._config0
        cfg1 = self._config1
        if camera_index == 0 and cfg0 is None:
            return False
        if camera_index == 1 and cfg1 is None:
            return False
        w, h, fps, pre = (cfg0 if camera_index == 0 else cfg1)
        ...
        if camera_index == 0:
            self._cam0 = self._enc0 = self._circular0 = None
        else:
            self._cam1 = self._enc1 = self._circular1 = None
        ...
        try:
            if camera_index == 0:
                ...
                cfg = self._cam0.create_video_configuration(
                    main=main, lores=lores, encode="main", controls={"FrameRate": fps}
                )
                self._cam0.configure(cfg)
                self._circular0 = CircularOutput2(buffer_duration_ms=max(1000, int(pre * 1000)))
                self._enc0 = H264Encoder(bitrate=6_000_000)
                self._enc0.output = [self._circular0]
                self._cam0.start_recording(self._enc0)
                self._config0 = (w, h, fps, pre)
                ...
                main = {"size": (w, h), "format": "YUV420"}
                lores = {"size": LORES_SIZE, "format": "YUV420"}
                cfg = self._cam1.create_video_configuration(
                    main=main, lores=lores, encode="main", controls={"FrameRate": fps}
                )
                self._cam1.configure(cfg)
                self._circular1 = CircularOutput2(buffer_duration_ms=max(1000, int(pre * 1000)))
                self._enc1 = H264Encoder(bitrate=6_000_000)
                self._enc1.output = [self._circular1]
                self._cam1.start_recording(self._enc1)
                self._config1 = (w, h, fps, pre)
                logger.info("Camera 1 restarted %dx%d @ %d fps", w, h, fps)
                return True
        except Exception as e:
            logger.exception("Restart camera %d failed: %s", camera_index, e)
            return False
    ...
    def capture_frame(self, camera_index: int) -> Optional[bytes]:
        ...
        try:
            ...
            # YUV420: Y plane is first LORES_SIZE[1] rows, LORES_SIZE[0] cols
            y = arr[:h, :w] if arr.ndim >= 2 else arr
            img = Image.fromarray(y).convert("L")
            buf = io.BytesIO()
    ...
    def get_sensor_modes(self, camera_index: int) -> List[dict]:
        """Return list of supported (width, height, fps). Returns [] if not available."""
        cam = self._cam0 if camera_index == 0 else self._cam1
        if cam is None:
            return []
        try:
